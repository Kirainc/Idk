...
getgenv().InterfaceSett = {
            ColorSec1 = Color3.fromRGB(134, 126, 180), -- เปลี่ยนพวกสีตรงกรอบ
            ColorSec2 = Color3.fromRGB(134, 126, 180), -- เปลี่ยนสี toggle logo, slidebar
            WinBarSec1 = "Seraphic Hub | Premium Script", -- เปลี่ยนชื่อ win menu
            WinBarDate = 1, -- เปลี่ยนรูปแบบแสดงวัน
            InterfaceLogo = 83231088025993 -- โลโก้
        }
        ...
        function tab:CreateTap(txt)
            ...
            function sec:Section(Side)
                ...
                function main:AddToggle(name,options)
					...
					function BunnyGayhod:Update(states)
						if states then
                            TweenService:Create(
                                Check,
                                TweenInfo.new(0.3,Enum.EasingStyle.Quad,Enum.EasingDirection.Out),
...
local Window = lib:Win(getgenv().InterfaceSett.WinBarSec1)

-- สร้าง Tab
local MainTab = Window:CreateTap("Main")
local CombatTab = Window:CreateTap("Combat")

-- สร้าง Section ใน Tab (Left หรือ Right)
local LeftSec = MainTab:Section("Left")
local RightSec = MainTab:Section("Right")

-- เพิ่ม Elements
LeftSec:Title("Movement")

LeftSec:AddToggle("Fly", {
    Stats = false,
...
LeftSec:AddButton("Reset", function()
    game.Players.LocalPlayer.Character.Humanoid.Health = 0
end)

RightSec:Title("Misc")

RightSec:AddLabel("Player Info")

RightSec:AddTextBox("Name", {
    Placeholder = "ใส่ชื่อ...",
...
RightSec:Dropdown("Team", "None", false, {"Red","Blue","Green"}, function(selected)
    print("Selected:", selected)
end)

-- Section ใน Tab Combat
local CombatSec = CombatTab:Section("Left")

local aimbotToggle = CombatSec:AddToggle("Aimbot", {
    Stats = false,
    callback = function(value)
        print("Aimbot:", value)
    end
...
-- อัพเดท Toggle จากโค้ด
-- aimbotToggle:Update(true)
